...
function fnParse(row, index, rowColumns){
	row = d3.autoType(row);		//https://github.com/d3/d3-dsv#autoType
	...
	//return row;
	parsedRow = {};
	rowColumns.forEach((column) => {column.substring(0,2) == "\\s" ? parsedRow[column.substring(2)] = row[column].toString() : parsedRow[column] = row[column] });
    return parsedRow;
} //fnParse
