//My Warehouse Visualizer	
	...
	var appWarehouses = [{warehouseName:"Upload..."}];																					//Array of objects for demo and uploaded visuals
...
function fnShowMyWarehouseVisualizerDemo(){
	...
	if (!appWarehouses[warehouseIndex].layoutData){
		...
		d3.csv( "data/" + warehouseName + "/layout.csv", d3.autoType ) //https://github.com/d3/d3-fetch#csv
		//d3.csv( "data/" + warehouseName + "/layout.csv" ) //https://github.com/d3/d3-fetch#csv
		...
		function fnLoadWarehouseData(warehouseName, warehouseScene, layoutData){
			...
			d3.csv( "data/" + warehouseName + "/inventory.csv", d3.autoType ) //https://github.com/d3/d3-fetch#csv
	...
	function fnShowWarehouse(warehouseName, warehouseScene, inventoryData, layoutData){ 
		...
		if (inventoryData) {	
			...
			warehouseVisual.join.forEach( (join) => {join.visualObj.children[1].visible =  true;}  );
	...
	function fnBuildWarehouse(layoutData) {
		...
		for (var i = 0; i < layoutData.length; i++) {
				...
				*/		
				...
				var geometryKey = "W" +  layoutData[i]["WIDTH"] + "H" + layoutData[i]["HEIGHT"]  + "D" + layoutData[i]["DEPTH"] ;
				...
				if (geometriesMap.has(geometryKey)) {
					...
					var cubeGeometry =   new THREE.BoxGeometry(	layoutData[i]["WIDTH"],
																layoutData[i]["HEIGHT"], 
																layoutData[i]["DEPTH"]
				...
				//pallet.material.opacity = 0;		//Initially not visible	
				...
				slot.position.x = layoutData[i]["X"]; 
				slot.position.y = layoutData[i]["Z"]; 								
				slot.position.z =  layoutData[i]["Y"];  
...
function fnHandleFileUploads (event) {				
	...
	if (files.length == 2){
		...
		reader.onloadend = function() {			//https://w3c.github.io/FileAPI/#filereader-result
			...
			var warehouseName = csvFile.name;
			var warehouseName = fnParseGLFile(gltfFile);
			
			appWarehouses.push({	url: null,
								blob: gltfFile,
			...
			parentLayout.get("main").toolbar.get("warehouse").items.push({id: appWarehouses.length - 1, text: warehouseName, checked: true});
		...
		var warehouseName = fnParseGLFile(files[0]);		
		...
		appWarehouses.push({	url: null,
							blob: files[0],
							data: null,
							warehouseName: warehouseName + " (Embedded)",
							warehouseName: warehouseName
						});	
	...
	mdv.display = function(mdvLayout, dataVisual){
		...
		*/
		...
		function fnHandleSelectionOrSearch(event){
			...
			if (recIds.length > 0){
				...
				recIds.forEach(function (recid){
					...
					if (join) 
						join.visualObj.visible = true;    //Un-hide objects from the search
		...
		function fnGridSelectColumn(dataVisual){   //Used by data and warehouse grid to de-select Column (from either grid) and select on current grid
			...
			//Remove existing backgrounds from the complete data and warehouse grid; even if they are already applied to current column
		...
		function fnBuildScaleSlider(canvasParent, scaleTd, dataVisual) {
			...
			scaleSlider.addSlider({
				id: dataVisual.warehouseName + "_scaleSelector",
