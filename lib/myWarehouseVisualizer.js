...
function fnHandleFileUploads (event) {				
	...
	if (files.length != 2) { 
		...
		w2alert("Upload an Layout and Inventory file.");
		return;
	...
	if (!csvLayoutFile && !csvInventoryFile){
		...
		w2alert("Files must have 'Inventory' and 'Layout' in there names to distiguish them.")
...
function fnParse(row, index, rowColumns){
	...
	row = d3.autoType(row);
	rowColumns.forEach((column) => {row[column] = typeof row[column] != "string" ? row[column] : row[column].substring(0,2) == "\\s"  ?  row[column].substring(2) : row[column]  } );
	return row;
